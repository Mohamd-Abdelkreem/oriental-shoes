    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <span className="mt-1 block truncate text-sm text-slate-800">
      ...
      <div className="space-y-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between border-b pb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full border border-teal-200 bg-teal-50 px-2 py-0.5 text-xs font-bold text-teal-800">
              أحدث مقاس معتمد: {sizeProfile.latestSize || "غير مسجل"}
            </span>
          </div>
          <span className="text-xs text-slate-500">
            {sizeProfile.history.length} مقاسات مسجلة في أوامر سابقة
          </span>
        </div>
            ...
            <table className="w-full text-right text-xs">
              <thead className="border-b bg-slate-50 text-slate-600">
                <tr>
                  ...
                  <th className="px-3 py-2">رقم أمر التفصيل</th>
                  <th className="px-3 py-2">رقم القطعة</th>
                  <th className="px-3 py-2">الموديل</th>
                  <th className="px-3 py-2">المقاس المسجل</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {sizeProfile.history.map((h, idx) => (
                  <tr
                    key={`${h.orderId}-${String(idx)}`}
                    className="hover:bg-slate-50/60"
                  >
                    <td className="px-3 py-2 text-slate-500">{h.orderDate}</td>
                    <td
                      className="px-3 py-2 font-mono font-bold text-teal-700"
                      dir="ltr"
                    >
                      <Link
                    ...
                    <td className="px-3 py-2 text-slate-800">{h.model}</td>
                    <td className="px-3 py-2">
                      <span className="rounded border border-slate-200 bg-slate-100 px-2 py-0.5 text-xs font-bold text-slate-900">
                        {h.size}
                      </span>
                    </td>
                  </tr>
